...
import { SdkTplError } from '../errors/sdk-tpl-error';
...
describe('HttpClient', () => {
  ...
  it('throws SdkTplError on 4xx response', async () => {
    mockFetch.mockResolvedValue({
      ok: false,
      status: 422,
    ...
    await expect(client.request('/emails/send', { method: 'POST' })).rejects.toThrow(SdkTplError);

    try {
      await client.request('/emails/send', { method: 'POST' });
    } catch (err) {
      expect(err).toBeInstanceOf(SdkTplError);
      expect((err as SdkTplError).statusCode).toBe(422);
    }
  });

  it('throws SdkTplError on 5xx response', async () => {
    mockFetch.mockResolvedValue({
      ok: false,
      status: 500,
    ...
    await expect(client.request('/health', { method: 'GET' })).rejects.toThrow(SdkTplError);

    try {
      await client.request('/health', { method: 'GET' });
    } catch (err) {
      expect(err).toBeInstanceOf(SdkTplError);
      const sdkError = err as SdkTplError;
  ...
  it('throws NetworkError on fetch failure', async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(SdkTplError);
      expect((err as SdkTplError).code).toBe(ErrorCode.NETWORK_ERROR);
    }
  });
