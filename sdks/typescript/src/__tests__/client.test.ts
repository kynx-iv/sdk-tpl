...
import { SdkTplClient } from '../client';
...
describe('SdkTplClient', () => {
  ...
  describe('constructor', () => {
    it('throws if apiKey is empty', () => {
      expect(() => new SdkTplClient({ apiKey: '' })).toThrow();

      try {
        new SdkTplClient({ apiKey: '' });
      } catch (err) {
    ...
    it('throws if apiKey is not provided', () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      expect(() => new SdkTplClient({} as any)).toThrow();
    });

    it('creates client with valid config', () => {
      const client = new SdkTplClient({
  ...
  describe('healthCheck', () => {
    it('makes GET /health request', async () => {
      ...
      const client = new SdkTplClient({ apiKey: 'sdk_test_key_123' });
