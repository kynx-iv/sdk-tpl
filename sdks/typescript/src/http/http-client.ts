...
import { SdkTplError } from '../errors/sdk_tpl-error';
 ...
 *
 * Checks the `{{ENV_MODE_VAR}}` environment variable (Node.js) or the
...
export function getBaseUrl(): string {
  ...
  if (typeof process !== 'undefined' && process.env) {
    mode = process.env['{{ENV_MODE_VAR}}'];
  }

  // Browser / global fallback
  if (!mode && typeof globalThis !== 'undefined') {
    mode = (globalThis as Record<string, unknown>)['{{ENV_MODE_VAR}}'] as
      | string
      | undefined;
  }
      ...
      return this.circuitBreaker.execute<T>(async () => {
        ...
        try {
          ...
          if (!response.ok) {
            ...
            const apiError = SdkTplError.createErrorFromResponse(
              response.status,
              sanitizedBody,
              retryAfterHeader,
            );
          ...
          if (error instanceof SdkTplError) {
            throw error;
          }
          ...
          if (
            ...
            throw SdkTplError.TimeoutError(
          ...
          throw SdkTplError.NetworkError(networkMsg, cause);
        } finally {
          clearTimeout(timer);
