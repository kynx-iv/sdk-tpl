...
export interface SdkTplErrorOptions {
  statusCode?: number;
  recoverable?: boolean;
  retryAfter?: number;
  requestId?: string;
 ...
 *
 * > The class name `SdkTplError` is a placeholder.  The scaffolding tool
 * > will rename it to match the concrete SDK (e.g. `SdkTplError`).
 *
 ...
 */
export class SdkTplError extends Error {
  ...
  constructor(message: string, code: ErrorCode, options: SdkTplErrorOptions = {}) {
    super(message);

    // Maintain proper prototype chain for instanceof checks.
    Object.setPrototypeOf(this, new.target.prototype);

    this.name = 'SdkTplError';
    this.code = code;
   ...
   */
  static NetworkError(message: string, cause?: Error): SdkTplError {
    const details: Record<string, unknown> = {};
    if (cause) {
      details.cause = cause.message;
      details.causeName = cause.name;
    }
    return new SdkTplError(message, ErrorCode.NETWORK_ERROR, {
      recoverable: true,
      details,
    });
  }
  ...
  static AuthenticationError(
    ...
    return new SdkTplError(message, code, {
      recoverable: false,
    });
  }
   ...
   */
  static TimeoutError(message: string): SdkTplError {
    return new SdkTplError(message, ErrorCode.NETWORK_TIMEOUT, {
      recoverable: true,
    });
  }
  ...
  static createErrorFromResponse(
    ...
    // 401 Unauthorized
    if (status === 401) {
      return new SdkTplError(serverMessage, ErrorCode.AUTH_UNAUTHORIZED, {
        statusCode: status,
        recoverable: false,
        requestId,
        details: parsed,
    ...
    if (status === 403) {
      return new SdkTplError(serverMessage, ErrorCode.AUTH_INVALID_KEY, {
        statusCode: status,
        recoverable: false,
        requestId,
        details: parsed,
    ...
    if (status === 429) {
      ...
      return new SdkTplError(serverMessage, ErrorCode.NETWORK_RETRY_LIMIT, {
        statusCode: status,
        recoverable: true,
        retryAfter,
        requestId,
    ...
    if (status >= 500) {
      ...
      return new SdkTplError(serverMessage, code, {
        statusCode: status,
        recoverable: true,
    ...
    // 4xx catch-all
    return new SdkTplError(serverMessage, ErrorCode.VALIDATION_ERROR, {
      statusCode: status,
      recoverable: false,
      requestId,
      details: parsed,
