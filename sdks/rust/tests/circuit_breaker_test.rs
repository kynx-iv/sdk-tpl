use sdk_tpl::errors::{ErrorCode, SdkTplError};
use sdk_tpl::http::circuit_breaker::{CircuitBreaker, CircuitState};
...
async fn test_circuit_breaker_closed_allows_requests() {
    ...
    let result = cb.execute(|| async { Ok::<i32, SdkTplError>(42) }).await;
...
async fn test_circuit_breaker_opens_after_threshold() {
    ...
    let result = cb.execute(|| async { Ok::<_, SdkTplError>(()) }).await;
    assert!(result.is_err());
    match result.unwrap_err() {
        SdkTplError::CircuitBreakerOpen { .. } => {}
...
async fn test_circuit_breaker_resets_on_success() {
    ...
    let _ = cb.execute(|| async { Ok::<_, SdkTplError>(()) }).await;
    assert_eq!(cb.state(), CircuitState::Closed);
}
