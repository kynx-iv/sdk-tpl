...
pub async fn with_retry<F, Fut, T>(max_retries: u32, operation: F) -> Result<T, SdkTplError>
where
    F: Fn() -> Fut,
    Fut: Future<Output = Result<T, SdkTplError>>,
{
    let mut last_error: Option<SdkTplError> = None;
    ...
    Err(last_error.unwrap_or_else(|| SdkTplError::Unknown {
...
mod tests {
    ...
    async fn test_with_retry_succeeds_immediately() {
        let result = with_retry(3, || async { Ok::<_, SdkTplError>(42) }).await;
