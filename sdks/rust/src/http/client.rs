...
impl HttpClient {
    ...
    pub fn new(config: &SdkTplConfig) -> Result<Self, SdkTplError> {
        ...
        default_headers.insert(
            USER_AGENT,
            HeaderValue::from_str(&format!("sdk_tpl-rust/{}", SDK_VERSION))
                .unwrap_or_else(|_| HeaderValue::from_static("sdk_tpl-rust")),
        );
        ...
        B: Serialize + Sync,
    {
        ...
        let body_json = match body {
            Some(b) => Some(serde_json::to_value(b).map_err(|e| SdkTplError::Validation {
            ...
            async move {
                cb.execute(|| async {
                    let req_method = method_clone
                        ...
                        .map_err(|_| SdkTplError::Validation {
                    ...
                    let response = request.send().await.map_err(|e| {
                        let err = SdkTplError::from_reqwest(e);
                    ...
                    if status >= 400 {
                        ...
                        let err = SdkTplError::from_status_with_retry_after(
                            status,
                            &body_text,
                            retry_after_secs,
                            request_id,
