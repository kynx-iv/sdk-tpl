...
/// The primary error type for the {{SDK_NAME}} SDK.
///
/// All public methods return `Result<T, SdkTplError>`.
#[derive(Debug, thiserror::Error)]
pub enum SdkTplError {
...
impl SdkTplError {
    ...
    /// Creates a [`SdkTplError::Network`] from a `reqwest::Error`.
    ...
    /// Creates a [`SdkTplError`] from an HTTP status code and body.
    pub fn from_status(status: u16, body: &str) -> Self {
        Self::from_status_with_retry_after(status, body, None, None)
    }

    /// Creates a [`SdkTplError`] from an HTTP status code, body, an
...
mod tests {
    ...
    #[test]
    fn test_from_status() {
        let err = SdkTplError::from_status(401, "");
        assert_eq!(err.error_code(), ErrorCode::Authentication);

        let err = SdkTplError::from_status(429, "");
        assert_eq!(err.error_code(), ErrorCode::RateLimited);

        let err = SdkTplError::from_status(503, "down");
