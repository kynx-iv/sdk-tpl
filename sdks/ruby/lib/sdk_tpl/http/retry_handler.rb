...
module SdkTpl
  module Http
    ...
    class RetryHandler
      ...
      # but honours +retry_after+ values carried on {SdkTplError} instances.
      #
      ...
      def execute(&block)
        ...
        (0..@max_retries).each do |attempt|
          begin
            ...
            delay = if e.is_a?(SdkTplError) && e.retry_after && e.retry_after > 0
        ...
        raise last_error || SdkTplError.new(
      ...
      # @return [Boolean]
      def retryable?(error)
        return false unless error.is_a?(SdkTplError)
