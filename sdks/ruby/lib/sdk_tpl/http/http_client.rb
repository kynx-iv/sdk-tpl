...
module SdkTpl
  module Http
    ...
    # breaking for the SdkTpl Ruby SDK.
    class HttpClient
      ...
      def initialize(api_key, config)
        ...
        @connection = Faraday.new(url: config.base_url) do |f|
          ...
          f.headers["X-SDK-Version"] = SdkTpl::VERSION
          f.headers["User-Agent"] = "sdk_tpl-ruby/#{SdkTpl::VERSION}"
      ...
      def perform_request(method, path, body, extra_headers)
        ...
        unless (200..299).cover?(status)
          ...
          raise SdkTplError.from_response(status, body_text, request_id: request_id, retry_after: retry_after_secs)
        end
        ...
        raise SdkTplError.timeout_error(msg)
      rescue Faraday::ConnectionFailed => e
        msg = "Network error during #{method} #{path}"
        msg = ErrorSanitizer.sanitize(msg) if @config.enable_error_sanitization
        raise SdkTplError.network_error(msg, cause: e)
      rescue SdkTplError
        raise
