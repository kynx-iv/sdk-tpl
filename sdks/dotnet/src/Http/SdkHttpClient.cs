...
using SdkTpl.Sdk.Errors;
using SdkTpl.Sdk.Security;
using SdkTpl.Sdk.Utils;

namespace SdkTpl.Sdk.Http;
...
internal sealed class SdkHttpClient : IDisposable
{
    ...
    private readonly SdkTplConfig _config;
    ...
    public SdkHttpClient(SdkTplConfig config)
    {
        _config = config;
        ...
        CancellationToken ct)
    {
        return await _retryHandler.ExecuteAsync(async () =>
        {
            return await _circuitBreaker.ExecuteAsync(async () =>
            {
                ...
                {
                    throw SdkTplException.TimeoutError("Request timed out", ex);
                }
                ...
                if (response.IsSuccessStatusCode)
                {
                    return JsonSerializer.Deserialize<T>(body, JsonOptions)
                        ?? throw SdkTplException.NetworkError("Failed to deserialize response");
                }
                ...
                if ((int)response.StatusCode == 401 && _config.SecondaryApiKey is not null)
                {
                    ...
                    throw SdkTplException.KeyRotationError(
                ...
                throw SdkTplException.FromResponse(
