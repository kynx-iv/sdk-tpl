...
public sealed class RetryHandler
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        for (var attempt = 0; attempt <= _maxRetries; attempt++)
        {
            ...
            catch (SdkTplException ex) when (ex.Recoverable && attempt < _maxRetries)
            {
                lastException = ex;
            ...
            {
                lastException = SdkTplException.NetworkError(ex.Message, ex);
            ...
            {
                lastException = SdkTplException.TimeoutError("Request timed out", ex);
                var delay = CalculateDelay(attempt, retryAfter: null);
                await Task.Delay(delay, cancellationToken).ConfigureAwait(false);
            }
        }

        throw lastException ?? SdkTplException.NetworkError("Operation failed after all retry attempts");
    }
