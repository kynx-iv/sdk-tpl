...
import (
	...
	internalhttp "github.com/{{ORG_SLUG}}/{{SDK_SLUG}}-go/internal/http"
	"github.com/{{ORG_SLUG}}/{{SDK_SLUG}}-go/types"
)

// Client is the main SdkTpl API client. Create one using NewClient.
type Client struct {
	httpClient *internalhttp.Client
	config     config.Config
}

// NewClient creates a new SdkTpl API client with the provided API key
...
// HealthCheck performs a health check against the SdkTpl API.
